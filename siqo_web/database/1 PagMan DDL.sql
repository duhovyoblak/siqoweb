...
--  = Zoznam objektov, používaných v SPAGE a menežovaných PageMan-om.\nObjekty môžu byť rôzneho druhu, napr. stránka, objekt na stránke, proces, atď.\nObjekt je základnou jednotkou riadenia práv.
;
...
CREATE TABLE IF NOT EXISTS SUSER (
  ...
  ,C_FUNC       CHAR(1)          DEFAULT 'L'             --  Users account status:\n\nW waiting for authentification\nA active\nL locked\nD deleted,
...
CREATE TABLE IF NOT EXISTS SSESSION (
   SESSION_ID   VARCHAR(32)      NOT NULL                --  Sssion ID vygenerované PHP,
  ,C_FUNC       CHAR(1)          DEFAULT 'E'             --  Stav session\n\nA aktívna\nE exspirovaná\nR zrušená pre nesúlad NK,
...
CREATE TABLE IF NOT EXISTS SOBJ_USER_ROLE (
  ...
  ,C_FUNC       CHAR(1)          DEFAULT 'A'            --  Stav autorizacie pre rolu\n\nA aktivna\nN neaktivna,
  ,D_CREATED    TIMESTAMP        NOT NULL
...
CREATE TABLE IF NOT EXISTS SOBJ_CACHE (
   USER_ID      VARCHAR(32)      NOT NULL DEFAULT 'Anonymous'
  ,PAGE_ID      VARCHAR(32)      NOT NULL
  ,OBJ_ID       VARCHAR(32)      NOT NULL
...
CREATE TABLE IF NOT EXISTS SDMS (
  ...
  ,N_SIZE       INT UNSIGNED         NULL               --  Velkost dokumentu v Byte,
