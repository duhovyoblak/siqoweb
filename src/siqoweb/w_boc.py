#==============================================================================
#                                                             (c) SIQO 25
# Class Boc for SIQO Flask application
#
#------------------------------------------------------------------------------
import re
import traceback
...
from   siqoweb.w_boc_f          import BocForm
...
class Boc(Window):
    ...
    def __init__(self, journal, dms, userId, lang, item, POST, idx):
        ...
        journal.I(f"Boc.__init__: From {item} for {userId}")
        ...
        self.winClass  = 'Boc'           # Window's class for response generator
    ...
    def resolvePost(self):
        ...
        #----------------------------------------------------------------------
        # Skontrolujem existenciu POST data
        #----------------------------------------------------------------------
        if self.POST is None:
            
            self.journal.M(f"{self.name}.resolvePost    : POST data does not exists", True)
            self.journal.O()
            return
...
#!!!!
    ...
    def formFromPost(self):
        ...
        if self.dbData is None:
            ...
            self.journal.M(f"{self.name}.resolvePost    : dbData data does not exists", True)
            self.journal.O()
            return
        ...
        #----------------------------------------------------------------------
        self.journal.M(f"{self.name}.formFromPost   : formType='BocForm', userId={self.dbData['USER_ID']}, itemId={self.idx}", True)

        try   : self.form = BocForm(formdata=self.POST, formType="BocForm", userId=self.dbData['USER_ID'], itemId=str(self.idx))
